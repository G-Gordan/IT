...
<head>
  ...
  <title>Blog Project - Comment List</title>
            ...
            <div class="card">
              <div class="card-header">
                ...
                <div class="card-tools">                    
                  <a href="comment-form" class="btn btn-success">
                ...
                <form class="center-block" role="form" action="comment-sort">                   
                 <table class="table table-bordered">                
                  <tbody id="sort-list">                  
                    <tr>
                      <td style="width: 90%">                      
                           <select class="form-control" id="sel1" name="orderBy">
                          <option value="0">All Comment By Id</option> <!-- sortira po id-ju kao da je vreme kreiranja -->
                           <option value="1">Sort By Post Id</option>
                           <option value="2">Sort By Commentator Name</option>
                           <option value="3">Sort Newest First</option>
                           </select>
                      </td>
                      <td class="text-center">                                                                          
                           <input class="btn btn-warning dropdown-toggle" type="submit" value="Sort">                      
                      </td>                      
                    </tr>
                  </tbody>
                 </table>
              ...
              <div class="card-body">              
                <table class="table table-bordered">                
                  <thead>                  
                    <tr>
                      <th style="width: 7%">Com. id</th>
                      <th style="width: 7%;">Blog id</th>
                       <th style="width: 14%;">Blog Author</th>
                      <th style="width: 40%;">Comment Text</th>
                      <th style="width: 12%;">Comm. Name</th>
                      <th style="width: 13%;">Status (click on)</th>                    
                  ...
                  <c:forEach var="comment" items="${commentList}">
                    <tr>
                      ...
                      <td>
                        ${comment.blog.author.name} ${comment.blog.author.surname}
                      </td>
                      <td>
                        ${comment.comment}
                      </td>
                      ...
                      <!-- enabled je boolean, if prolazi ako nije enabled== true (ili == 0) -->
                     <c:if test="${comment.enabled}">                        
                        <!-- ubaceno iz admin/index.html  -->
                        <!-- kad se klikne, poziva se akcija: message-seen i salje id na koji je kliknuto -->
                    	<a href="comment-status?id=${comment.id}" class="btn btn-block btn-success btn-flat">ENABLED</a>
                 	 </c:if>

                     <c:if test="${!comment.enabled}">                        
                        <!-- ubaceno iz admin/index.html  -->
                        <!-- kad se klikne, poziva se akcija: message-seen i salje id na koji je kliknuto -->
                    	<a href="comment-status?id=${comment.id}" class="btn btn-block btn-danger btn-flat">DISABLED</a>
                 	 </c:if>
