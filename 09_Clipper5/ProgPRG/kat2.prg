...
***Broji koliko ima indeksnih baza***
 IF m2 = "NTX"
  p_brojind:=p_brojind+1
 ENDIF
...
***Puni matrice MTRIND-imenom indeksne baze i MTRKLJ-kljucem te iste baze***
mtrime:=ARRAY(p_brojind)
mtrklj:=ARRAY(p_brojind)
USE podkat INDEX pkeksind
SEEK "NTX"
IF ! FOUND()
 ? "NEMA NI JEDNE INDEKSNE BAZE !"
 INKEY(0)
 ERASE podkat.dbf
 ERASE podbaza.dbf
 ERASE pknazind.ntx
...
ENDIF
p_brntx:=1
DO WHILE p_brntx<=p_brojind
 p_indime:=nazbppk
 mtrime[p_brntx]:=p_indime
 SKIP
 p_brntx:=p_brntx+1
ENDDO
USE
p_brntx:=1
FOR i:=1 TO p_brojind
 p_imeindbz:=mtrime[p_brntx]
 USE podkat INDEX &p_imeindbz
 p_indklj:=INDEXKEY(1)
 mtrklj[p_brntx]:=p_indklj

* ? ALLTRIM(STR(p_brntx))
* ??  " - "+ALLTRIM(mtrime[p_brntx])+" --- "+UPPER(mtrklj[p_brntx])
  p_brntx:=p_brntx+1
 USE
NEXT
...
DO WHILE ekst == "DBF"
 ...
 USE osobbp
 p_brslosbp:=RECCOUNT()
 FOR i1:=1 TO p_brslosbp

  p_imepolj:=" "
  p_imeklj:=" "
  p_imeindbz:=" "
  p_imepolj:=ALLTRIM(field_name)
  p_imepolj:=LOWER(p_imepolj)
  p_brntx:=1
  p_upis:=0
  p_nema:="NEMA_INDBZ"
  FOR i2:=1 TO p_brojind
   p_imeklj:=ALLTRIM(mtrklj[p_brntx])
   IF p_imepolj == p_imeklj
    p_imeindbz:=mtrime[p_brntx]
*?   p_brntx
*??  p_imepolj
*??  p_imeklj
*??  p_imeindbz
    ...
    SELECT 2
    APPEND BLANK
    REPLACE nazbppb WITH A->nazbppk, imepolja WITH D->field_name,;
            tippolja WITH D->field_type, brmesta WITH D->field_len,;
            brdecimala WITH D->field_dec, nazivind WITH p_imeindbz
  ...
  IF p_upis = 0
    ...
    REPLACE nazbppb WITH A->nazbppk, imepolja WITH D->field_name,;
            tippolja WITH D->field_type, brmesta WITH D->field_len,;
            brdecimala WITH D->field_dec, nazivind WITH p_nema
   ENDIF
